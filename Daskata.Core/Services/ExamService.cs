...
namespace Daskata.Core.Services
{
    public class ExamService : IExamService
    {
        ...
        {
            List<Exam> exams = await _repository.All<Exam>().ToListAsync();
        ...
        {
            var myExams = await _repository.All<Exam>()
        ...
        public async Task<Exam> Publish(FullExamViewModel model, string subject, string grade, Guid userId)
        {
        ...
        public async Task<Exam> GetExamByUrlAsync(string examUrl)
        {
            return await _repository.All<Exam>().FirstOrDefaultAsync(e => e.ExamUrl == examUrl);
        }

        public async Task<Exam> UpdateExamAsync(FullExamViewModel model)
        {
            var exam = await _repository.All<Exam>().FirstOrDefaultAsync(e => e.ExamUrl == model.ExamUrl);
        ...
        {
            var currentExam = await _repository.All<Exam>().FirstOrDefaultAsync(e => e.ExamUrl == examUrl);
